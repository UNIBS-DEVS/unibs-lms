...
class SessionAttendanceController extends Controller
{
    ...
    public function store(Request $request, BatchSession $session)
    {
        ...
        foreach ($request->attendance as $learnerId => $data) {
            ...
            // $isPresent = isset($data['present']) ? 'is_present' : 'absent';
            $isPresent = isset($data['present']);
    ...
    public function sendAttendanceEmail(BatchSession $session)
    {
        ...
        $clientCode = (string) ($session->batch->client_code ?? '');

        if (empty($clientCode)) {
            return back()->with('error', 'Client code missing.');
        }
        ...
        Mail::to($customerEmail)
            ...
            ->later(
                now()->addSeconds(10),
                new SessionAttendanceMail($session->id, $clientCode)
            );
